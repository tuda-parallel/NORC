...
set_noise_pattern(){
  ...
  if [ ! "$1" = "$current_noise_pattern" ]; then
    ...
    if [ ! "$current_noise_pattern" = "NO_NOISE" ]; then
      ...
      OMP_NUM_THREADS=1 srun -n $((§noise_procs * §nodes)) -c 1 --ntasks-per-node=§noise_procs --overcommit --overlap --cpu-bind=verbose,map_cpu:§odd_cpus NOIGENA PATTERN_$current_noise_pattern &
...
srun -n $((§noise_procs * §nodes)) -c 1 --ntasks-per-node=§noise_procs --overcommit --overlap --cpu-bind=verbose,map_cpu:§odd_cpus bash -c 'echo "Host: $(hostname), Task: $SLURM_PROCID, Cpus_allowed_list: $(grep Cpus_allowed_list /proc/self/status)"'
next_job_step=$((next_job_step + 1))

echo "BENCHMARK CPU ASSIGNMENT"
srun -n $((§procs * §nodes)) -c §threads --ntasks-per-node=§procs --overcommit --overlap --cpu-bind=verbose,mask_cpu:§even_cpus_mask bash -c 'echo "Host: $(hostname), Task: $SLURM_PROCID, Cpus_allowed_list: $(grep Cpus_allowed_list /proc/self/status)"'
next_job_step=$((next_job_step + 1))
